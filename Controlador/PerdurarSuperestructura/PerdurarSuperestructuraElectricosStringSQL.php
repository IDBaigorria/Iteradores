 ...
 */
class PerdurarSuperestructuraElectricosStringSQL extends Objeto implements PerdurarSuperestructura
{

    /**
     * @var string Token de seguridad recibido de la clase NodoElectrico.
     */
     ...
     *
     * @param string $token Token de seguridad proporcionado por NodoElectrico.
     * @return void
     */
	...
	static private function crear_consulta_insertar_sql($nombre)
	{
		...
		$datos = NodoElectrico::por_cada_nodo_ejecutar(static::$token, function ($nodo) {
	...
	static public function cargar($nombre): bool|null
	{
		...
		NodoElectrico::vaciar_superestructura(static::$token);
		$equivalencias = array();
		while ($nodo != null) {
			$id = $nodo["idnodo"];
			if (self::es_id_especial($id)) {
				//echo "ppp".$id."klj";
				if (!$naux = NodoElectrico::nodo_por_id($id)) {
					NodoElectrico::crear_con_dato_e_id(/*utf8_decode(*/ $nodo["dato"]/*)*/ , $id);
				} else {
					$naux->_dato(/*utf8_decode(*/ $nodo["dato"]/*)*/);
				}
			} else {
				$idnuevo = NodoElectrico::crear_con_dato(/*utf8_decode(*/ $nodo["dato"]/*)*/)->id();
